...
#if defined(MQTTS)
	NULL, 		/**< list of clients connected via MQTTS */
	NULL,			/**< list of disconnected clients for MQTTS */
#endif
...
char* Broker_recordFFDC(char* symptoms)
{
	...
	if (strncmp(symptoms, "Requested by command.", strlen("Requested by command.")) != 0)
		BrokerState.ffdc_count++;
